...
public class FilmDAO {
    ...
    public Films getFilmsById(int id) throws SQLException {
        ...
        try {
            ...
            if (rs.next()) {
                //films = new Films();
                films = new Films(id, rs.getString("fName"), rs.getInt("pId"), rs.getDate("releaseDate"), rs.getDouble("rating"), rs.getInt("limitAge"), rs.getInt("status"), rs.getDate("airDate"), rs.getDate("endDate"));
            }
        } catch (Exception e) {
        }
